...
4. **Write distractors first. Always.**
   ...
   f. **Per-item mechanical rule (MUST follow on every single MC item):** After writing the correct answer, count its character length. At least ONE distractor in the same item MUST have a character count greater than or equal to the correct answer's character count. Verify this BEFORE moving on to the next item. This is a checkable mechanical rule, not a soft guideline — the easiest way to satisfy it is to write the longest distractor first, then the correct answer to match it.
   g. Across the 50 MC questions in this file, the correct answer should be the longest option in no more than 25% of questions (+5% tolerance = 30% hard cap). The VALIDATOR WILL REJECT files exceeding 30%.
   h. THE VALIDATOR WILL REJECT the file if any of: (i) any single question violates the parity rule (±20% words / ±30% chars); (ii) the correct answer is the longest in more than 30% of MC questions (`MC_SYSTEMATIC_LENGTH_BIAS`); (iii) more than 10% of MC items have a correct option whose character length exceeds the longest distractor by more than 8 characters AND is more than 30% above the average distractor (`MC_CHAR_LENGTH_BIAS`). Rule (iii) catches the full-sentence-vs-short-phrase pattern that word-count parity alone misses.
   i. **Self-check before reporting done.** Run `python3 questions/_validation/validate.py <output-file>` as the final step of generation. If it reports any errors — particularly `MC_CHAR_LENGTH_BIAS`, `MC_SYSTEMATIC_LENGTH_BIAS`, or `MC_OPTION_PARITY` — fix the offending items and re-run before declaring the file complete.
